        ...
        try {
            ...
            if (errorMessage != null) {
                map.put("errorMessage", errorMessage);
					...
					// +++++ custom start +++++
					String realm = (String) themeProperties.getOrDefault("baseRealm", "ijug");
					session.getContext().setRealm(session.realms().getRealmByName(realm));
					IdentityProviderStorageProvider provider = session.getProvider(IdentityProviderStorageProvider.class);
					List<IdentityProviderModel> idps = provider.getAllStream(IdentityProviderQuery.userAuthentication()).toList();
					map.put("jugs", idps);
					// +++++ custom end +++++
